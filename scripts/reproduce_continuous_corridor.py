#!/usr/bin/env python3
"""Run continuous-corridor Bucket QR-MAX suites."""
...
DEFAULT_CONFIG = PROJECT_ROOT / "configs" / "continuous_corridor_bucket_qrmax.json"
DEFAULT_OUTPUT = (
    PROJECT_ROOT / "paper_results" / "continuous_corridor_bucket_qrmax_summary.csv"
)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--suite", default="continuous_corridor_smoke")
...
def _iter_jobs(config: dict, suite_name: str, overrides: argparse.Namespace):
    ...
    for group in suite.get("runs", []):
        ...
        buckets_y_values = _selected(
            overrides.buckets_y,
            group.get("buckets_y", defaults.get("buckets_y", [4])),
        )
    ...
    for job in _iter_jobs(config, args.suite, args):
        ...
        if not args.dry_run:
            if run_bucket_qrmax_sweep is None:
                from multiagent_rlrm.environments.continuous_corridor import (
